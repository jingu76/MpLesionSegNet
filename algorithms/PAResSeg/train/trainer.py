...
def train_epoch(model, loader, optimizer, scaler, epoch, loss_func, args):
    ...
    for idx, batch_data in enumerate(loader):
        if isinstance(batch_data, list):
            ...
            data, target = batch_data["image"], batch_data["label"]  # (b, s, d, h, w), (b, s, d, h, w)
        data, target = data.cuda(args.rank), target.cuda(args.rank)
        target = target[:, 2:3, 1:2]  # (b, c, d, h, w)
        for param in model.parameters():
            param.grad = None
        with autocast(enabled=args.amp):
            logits = model(data)  # (b, c, d, h, w)
...
def val_epoch_v2(model, loader, epoch, acc_func, args, model_inferer=None, post_label=None, post_pred=None):
    ...
    with torch.no_grad():
        for idx, batch_data in enumerate(loader):
            ...
            data, target = data.cuda(args.rank), target.cuda(args.rank)  # (b, c, d, h, w)
            target = target[:, 2:3]
            with autocast(enabled=args.amp):
