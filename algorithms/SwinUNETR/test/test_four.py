    ...
    with torch.no_grad():
        for i, batch in enumerate(val_loader):
            ...
            if args.use_tta:
                ...
                tta_infer = tta.SegmentationTTAWrapper(model_inferer, tta.aliases.d4_transform(), merge_mode='mean')
                val_outputs = tta_infer(val_inputs)
            else:
            ...
            if args.save_output:
                nib.save(
                    nib.Nifti1Image(val_outputs.astype(np.uint8), original_affine), os.path.join(args.output_directory, img_name)
                )
