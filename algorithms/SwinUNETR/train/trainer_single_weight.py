...
def train_epoch(model, loader, optimizer, scaler, epoch, loss_func, args):
    ...
    for idx, batch_data in enumerate(loader):
        if isinstance(batch_data, list):
            ...
            data, target, weight = batch_data["image"], batch_data["label"], batch_data['weight']
        data, target, weight = data.cuda(args.rank), target.cuda(args.rank).long(), weight.cuda(args.rank) / 255.
        # target, weight: b, 1, h, w, d
        for param in model.parameters():
            param.grad = None
        with autocast(enabled=args.amp):
            logits = model(data)  # logits: b, c, h, w, d
