...
class URIM(nn.Module):
    def __init__(self, num_classes=2):
        super().__init__()
        self.softmax = nn.Softmax(dim=1)
        self.lc_conv_1 = nn.Conv2d(in_channels=256 + num_classes, out_channels=128, kernel_size=3, padding=1)
        ...
        self.classifier = nn.Conv2d(in_channels=128, out_channels=num_classes, kernel_size=1, padding=0)
...
class Network(nn.Module):
    ...
    def forward(self, x):
        ...
        # return final_seg.unsqueeze(2), initial_seg.unsqueeze(2)
        return final_seg.unsqueeze(2)


if __name__ == '__main__':
    import ssl
    ssl._create_default_https_context = ssl._create_unverified_context
    m = Network(num_classes=4)
    x = torch.rand((10, 4, 3, 512, 512))
    logits = m(x)
    print(logits.shape)